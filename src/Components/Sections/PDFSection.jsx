...
const DropDown = ({ title, options }) => {
  ...
  return (
    <div className='relative'>
      <div
        className='transition-all p-1 lg:p-5 rounded duration-300 cursor-pointer '
        onMouseEnter={() => setIsHovered(true)}
        onMouseLeave={() => setIsHovered(false)}
      >
        <span className=' text-sm lg:text-xl p-3 lg:p-5 rounded hover:bg-black hover:text-orange-500'>
          {title}
        </span>
      </div>
      {isHovered && (
